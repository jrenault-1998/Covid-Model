...
#creating the matrix
CT <- matrix(0, nrow = t+1, ncol = 10)
colnames(CT) <- c("S", "E", "I_p", "I_c", "I_a", "Q", "Q_a", "S_q", "R", "Q_f")
CT[1, "S"] = N
CT[2, "S"] = N
CT[3, "S"] = N
CT[4, "S"] = N
...
output <- matrix(0,nrow = length(q.vec), ncol=length(alpha.vec))
    ...
    for (i in seq(5,t-1) ) {
      ...
      CT[i+1, "Q_f"] = Q_f +  (E4*r*deltaE) * q * (1-alpha) * c * (b_c*(S+S1)+(S2+S3))/N + (E4*r*deltaE) * c * q * alpha * (1-r) * (b_c*(S+S1)+(S2+S3))/N + (E4*r*deltaE) * c * q * alpha * r * (b_c*(S+S1)+(S2+S3))/N
    }
    
diff.target = unname(CT[i+1,"Q_f"]) - target
